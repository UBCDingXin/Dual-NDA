...
'''                                Output folders                                  '''
#######################################################################################
path_to_output = os.path.join(wd, 'output/CcGAN_{}_{}_si{:.3f}_ka{:.3f}_{}_nDs{}_nDa{}_nGa{}_Dbs{}_Gbs{}'.format(args.GAN_arch, args.threshold_type, args.kernel_sigma, args.kappa, args.loss_type_gan, args.num_D_steps, args.num_grad_acc_d, args.num_grad_acc_g, args.batch_size_disc, args.batch_size_gene))
...
if not os.path.isfile(ckpt_gan_path):
    if args.GAN_arch == "SAGAN":
        netG = sagan_generator(nz=args.dim_gan, dim_embed=args.dim_embed).cuda()
        netD = sagan_discriminator(dim_embed=args.dim_embed).cuda()
    else:
        netG = sngan_generator(nz=args.dim_gan, dim_embed=args.dim_embed).cuda()
        netD = sngan_discriminator(dim_embed=args.dim_embed).cuda()
...
if args.mae_filter:
    ...
    def compute_mae(net, input_images, input_labels, batch_size=args.samp_batch_size):
        ''' 
    ...
    for i in trange(num_target_labels):
        ...
        mae_cutoff_point_i = np.quantile(mae_i, q=args.samp_filter_mae_percentile_threshold)
        mae_cutoff_point_all.append(mae_cutoff_point_i)
    ## for i
    mae_cutoff_point_all = np.array(mae_cutoff_point_all)

    ## plot of MAEs
    fig = plt.figure()
    ax = plt.subplot(111)
    ax.plot(fn_denorm_labels(target_labels_norm), fn_denorm_labels(mae_cutoff_point_all), color='tab:red', linestyle='-', marker='o', alpha=0.85)
    ax.set_xlabel("Cutoff Points", fontsize=12)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(num_target_labels)):
            ...
            while ngot_<args.samp_filter_nfake_per_label:
                ...
                indx_sel = np.where(batch_loss>=mae_cutoff_point_all[i])[0]
    ...
    selected_labels = np.linspace(start_label, end_label, num=n_row).astype(int)
    indx_show = []
    for i in range(len(selected_labels)):
