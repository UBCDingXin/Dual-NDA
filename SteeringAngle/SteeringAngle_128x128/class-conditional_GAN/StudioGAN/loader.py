...
def load_worker(local_rank, cfgs, gpus_per_node, run_name, hdf5_path):
    ...
    if load_eval_dataset:
        ...
        def fn_norm_labels(input):
            ...
            output /= max_label_after_shift
            if type(output) is np.ndarray or type(output) is torch.Tensor:
                assert output.min()>=0 and output.max()<=1.0
            elif type(output) is float:
                assert output>=0 and output<=1.0
            return output
    ...
    if cfgs.RUN.do_eval:
        ...
        # Diversity: entropy of predicted scenes within each eval center
        PreNetDiversity = ResNet34_class_eval(num_classes=5, ngpu = torch.cuda.device_count()).cuda() #5 scenes
        ...
        eval_labels = np.linspace(np.min(labels_all), np.max(labels_all), cfgs.RUN.num_eval_labels) #not normalized
        ...
        fake_labels_assigned = fn_norm_labels(fake_labels_assigned)
        ...
        for i in range(len(centers_loc)):
            ...
            indx_real = np.where((real_labels>=fn_norm_labels(interval_start))*(real_labels<=fn_norm_labels(interval_stop))==True)[0]
            np.random.shuffle(indx_real)
            real_images_curr = real_images[indx_real]
            num_realimgs_over_centers[i] = len(real_images_curr)
            indx_fake = np.where((fake_labels_assigned>=fn_norm_labels(interval_start))*(fake_labels_assigned<=fn_norm_labels(interval_stop))==True)[0]
            ...
            # Label score
            labelscores_over_centers[i], _ = cal_labelscore(PreNetLS, fake_images_curr, fake_labels_assigned_curr, min_label_before_shift=min_label_before_shift, max_label_after_shift=max_label_after_shift, batch_size = cfgs.RUN.eval_batch_size, resize = None)
        ...
        ls_mean_overall, ls_std_overall = cal_labelscore(PreNetLS, fake_images, fake_labels_assigned, min_label_before_shift=min_label_before_shift, max_label_after_shift=max_label_after_shift, batch_size = cfgs.RUN.eval_batch_size, resize = None)
