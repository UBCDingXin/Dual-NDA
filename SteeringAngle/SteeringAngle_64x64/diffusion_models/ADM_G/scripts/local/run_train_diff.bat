...
set TRAIN_FLAGS=--lr 1e-5 --weight_decay 1e-3 --batch_size 58 --use_fp16 True --log_interval 500 --lr_anneal_steps 100000 --save_interval 5000

set CUDA_VISIBLE_DEVICES=1
python image_train.py ^
    --setup_name Setup1 ^
    --root_dir %ROOT_PATH% --data_dir %DATA_PATH% ^
    --resume_checkpoint .\output\exp_Setup1\diffusion\model050000.pt ^
    %MODEL_FLAGS% ^
    %DIFFUSION_FLAGS% ^
    %TRAIN_FLAGS% ^ %*
