...
if not args.gan_DiffAugment:
    path_to_output = path_to_output+'_NoDA'
...
if args.net_embed == "ResNet18_embed":
    net_embed = ResNet18_embed(dim_embed=args.dim_embed, ngpu = torch.cuda.device_count())
elif args.net_embed == "ResNet34_embed":
    net_embed = ResNet34_embed(dim_embed=args.dim_embed, ngpu = torch.cuda.device_count())
elif args.net_embed == "ResNet50_embed":
    net_embed = ResNet50_embed(dim_embed=args.dim_embed, ngpu = torch.cuda.device_count())
...
if not os.path.isfile(Filename_GAN):
    if args.GAN_arch=="SNGAN":
        netG = sngan_generator(nz=args.dim_gan, dim_embed=args.dim_embed)
        netD = sngan_discriminator(dim_embed=args.dim_embed)
    elif args.GAN_arch=="SAGAN":
        netG = sagan_generator(nz=args.dim_gan, dim_embed=args.dim_embed, gene_ch=64)
        netD = sagan_discriminator(dim_embed=args.dim_embed, disc_ch=50)
    ...
    if args.GAN_arch=="SNGAN":
        ...
        netG = sagan_generator(nz=args.dim_gan, dim_embed=args.dim_embed, gene_ch=64).cuda()
    netG = nn.DataParallel(netG)
...
if args.comp_FID:
    ...
    if args.comp_IS_and_FID_only:
        ...
        with open(eval_results_logging_fullpath, 'a') as eval_results_logging_file:
            ...
            eval_results_logging_file.write("\n NITERS: {}; Radius: {}; # Centers: {}.  \n".format(args.niters_gan, args.FID_radius, args.FID_num_centers))
