...
def train_ccgan(kernel_sigma, kappa, train_images, train_labels, netG, netD, net_y2h, save_images_folder, save_models_folder = None, clip_label=False):
    ...
    z_fixed = torch.randn(n_row*n_col, dim_gan, dtype=torch.float)
    ...
    print(y_fixed)
    y_fixed = torch.from_numpy(y_fixed).type(torch.float).view(-1,1)
    ...
    for niter in range(resume_niters, niters):
        ...
        for step_D_index in range(num_D_steps):
            ...
            for accumulation_index in range(num_grad_acc_d):
                ...
                # forward pass
                if use_DiffAugment:
                    real_dis_out = netD(DiffAugment(batch_real_images.to(device), policy=policy), net_y2h(batch_target_labels))
                    fake_dis_out = netD(DiffAugment(batch_fake_images.to(device).detach(), policy=policy), net_y2h(batch_target_labels))
                    if nda_b>0 and niter>=nda_start_iter:
                        real_fake_dis_out = netD(DiffAugment(batch_real_fake_images.to(device), policy=policy), net_y2h(batch_target_labels.to(device)))
                    if nda_c>0 and niter>=nda_start_iter:
                        real_wronglabel_dis_out = netD(DiffAugment(batch_real_wronglabel_images.to(device), policy=policy), net_y2h(batch_target_labels.to(device)))
                    if nda_d>0 and niter>=nda_start_iter:
                        badfake_dis_out_1 = netD(DiffAugment(batch_badfake_images_1.to(device), policy=policy), net_y2h(batch_target_labels.to(device)))
                    if nda_e>0 and niter>=nda_start_iter:
                        badfake_dis_out_2 = netD(DiffAugment(batch_badfake_images_2.to(device), policy=policy), net_y2h(batch_target_labels.to(device)))
                else:
        ...
        if (niter+1) % visualize_freq == 0:
            netG.eval()
            with torch.no_grad():
                gen_imgs = netG(z_fixed.to(device), net_y2h(y_fixed.to(device)))
